...
function all_projects = enrich(all_projects, en_tuple)
    ...
    for j = 1:length(all_projects)
        ...
        for i = 1:length(project_models)
            ...
            if model.m_num < 810  || model.m_num > 1000
                continue
            end

            if (strcmp(en_tuple{1}, C.CYCLOMATIC_COMP) && ismember(model.m_num, [242, 399, 817, 9006])) || ismember(model.m_num, [2718, 3705, 6150, 6151, 6807, 6808, 6809, 8839, 8931])
                continue
            end
